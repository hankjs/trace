...
ST 有两个字段，**只有一个能用于回测**：`quant_daily_bar.is_st` 是逐日真实历史
（baostock 日线的 `isST`），历史筛选与回测一律用它；`quant_stock.is_st` 只有当前
状态（改名即覆盖），仅供 UI 展示。

用当前状态过滤历史样本是系统性前视偏差 —— 实测抽样 8 只当前 ST 股，22464 个交易日
里真正处于 ST 的只有 14.4%，用当前标记会把其余 85.6% 一并剔除，而被剔掉的恰是后来
才出问题的公司，方向上高估策略表现。详见
[`DATA-ARCHITECTURE.md`](DATA-ARCHITECTURE.md) 第 4 节。

一处例外：当日选股（`run_selection`）按当前名称判定 ST 是正确的 —— 当日的当前状态
就是正确状态，不存在前视问题。

**`is_st` 回填已完成**（2026-07-30 核实：沪深 5457 / 5457 只零缺口）。北交所
330 只不在 baostock 覆盖内，历史行 `is_st` 仍为 `NULL`，严格口径下不参与历史全 A 池。
每晚调度会自动补新数据并回补最近 10 天；如需重跑用
`uv run python scripts/backfill_is_st.py`（单进程，严禁并发，见 DATA-ARCHITECTURE 第 4 节教训）。
