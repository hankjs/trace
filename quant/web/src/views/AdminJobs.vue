...
/**
 * 定时任务(admin 专属):查看调度状态并手动触发一次执行。
 *
 * 手动触发不绕过任务内部守卫:非交易日、盘中时间窗外等场景任务会自行
 * 跳过,表现为「已完成」但结果里 skipped/无输出。执行状态是后端进程
 * 内存态,服务重启后清空。
 */
import { computed, onBeforeUnmount, onMounted, ref } from 'vue'
import { Loader2, Play, RefreshCw, Timer } from 'lucide-vue-next'
    ...
    <section class="rounded border border-border bg-surface-raised">
      ...
      <div class="px-3 py-2.5 text-xs leading-5 text-text-secondary">
        ...
        <p class="mt-1 text-text-tertiary">
          手动执行不会绕过任务内部守卫:非交易日、盘中时间窗外等场景任务会自行跳过。执行状态保存在服务进程内存中,重启后清空。
        </p>
      </div>
    </section>
    ...
    <section class="overflow-hidden rounded border border-border bg-surface-raised">
      <div class="overflow-x-auto">
        <table class="w-full min-w-[760px] text-left text-xs">
          ...
          <tbody v-if="loading">
            <tr>
              <td colspan="5" class="px-3 py-6 text-text-tertiary">加载中…</td>
            </tr>
          </tbody>
          <tbody v-else>
            <tr
              ...
              <td class="px-3 py-2.5 align-top">
                <template v-if="job.manual_run">
                  <div class="flex items-center gap-1.5">
                    ...
                    <span
                      :class="{
                        'text-text-secondary': job.manual_run.status === 'running',
                        'text-text-primary': job.manual_run.status === 'finished',
                        'text-up': job.manual_run.status === 'failed',
                      }"
                    >{{ runSummary(job.manual_run) }}</span>
                  </div>
