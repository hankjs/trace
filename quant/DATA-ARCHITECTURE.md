...
- **一次性回填**：`scripts/backfill_is_st.py` 只 `UPDATE` 这一列、不碰价格列、不走重锚检查，按股票粒度断点续跑。**已于 2026-07-30 全部跑完**：沪深 5457 / 5457 只零缺口；北交所 330 只 baostock 不覆盖，`is_st` 留 `NULL`，严格口径下排除出历史全 A 池。
