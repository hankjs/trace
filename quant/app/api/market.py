...
def _stock_out(stock: Stock) -> dict:
    return {
        "code": stock.code,
        "name": stock.name,
        "industry": stock.industry,
        "is_watch": stock.is_watch,
    }
...
def search_stocks(
    ...
    if not query:
        stmt = stmt.where(Stock.is_watch.is_(True)).order_by(Stock.code)
    elif full_code:
        ...
        stmt = stmt.where(Stock.code.like(f"{prefix}%")).order_by(
            Stock.is_watch.desc(), Stock.code,
        )
    ...
    return {
        "query": query,
        "count": len(rows),
        "items": [_stock_out(stock) for stock in rows],
    }
