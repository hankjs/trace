          ...
          <template v-for="job in jobs" :key="job.id">
            <tr class="border-b border-border-subtle">
              <td class="py-2 pr-3 align-top">
                ...
                <div class="mt-0.5 max-w-64 text-xs leading-4 text-text-tertiary">{{ job.description }}</div>
                <div class="mt-0.5 font-mono text-[10px] text-text-tertiary">{{ job.id }}</div>
              </td>
              <td class="whitespace-nowrap py-2 pr-3 align-top text-xs text-text-secondary">{{ job.schedule }}</td>
              <td class="py-2 pr-3 align-top">
                <button type="button" @click="toggleEnabled(job)" class="flex min-h-9 items-center gap-2">
                  <span class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors" :class="job.enabled ? 'bg-green-500' : 'bg-border-subtle'">
                    <span class="inline-block h-3.5 w-3.5 rounded-full bg-white transition-transform" :class="job.enabled ? 'translate-x-4' : 'translate-x-0.5'"></span>
                  </span>
                </button>
              </td>
              ...
              <td class="whitespace-nowrap py-2 align-top">
                <button type="button" @click="toggleHistory(job)" class="mr-3 min-h-8 text-xs text-text-secondary hover:underline">
                ...
                <button
                  ...
                  class="min-h-8 text-xs text-accent hover:underline disabled:cursor-not-allowed disabled:opacity-40"
                >立即执行</button>
              </td>
            </tr>
