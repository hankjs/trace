...
async function load() {
  loading.value = true
  try {
    providers.value = await api.listImageProviders()
  } catch (e) { /* ignore */ }
  loading.value = false
}
...
async function save() {
  ...
  if (editingId.value) {
    await api.updateImageProvider(editingId.value, data)
  } else {
    await api.createImageProvider(data)
  }
  resetForm()
  await load()
}
...
async function moveUp(idx: number) {
  ...
  await api.updateImageProvider(curr.id, { ...toData(curr), priority: prev.priority })
  await api.updateImageProvider(prev.id, { ...toData(prev), priority: curr.priority })
  await load()
}
...
async function moveDown(idx: number) {
  ...
  const curr = items[idx]
  await api.updateImageProvider(curr.id, { ...toData(curr), priority: next.priority })
  await api.updateImageProvider(next.id, { ...toData(next), priority: curr.priority })
  await load()
}
...
<template>
  <div>
    <div class="mb-6 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
      <h1 class="text-lg font-semibold text-text-primary">生图供应商管理</h1>
      <button type="button" @click="showForm = true; editingId = null" class="min-h-10 self-start rounded-md bg-accent px-3 py-2 text-xs text-white hover:opacity-90 sm:min-h-0 sm:py-1.5">添加生图供应商</button>
    </div>
    ...
    <div v-if="showForm" class="mb-6 space-y-3 rounded-lg border border-border-subtle p-4">
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div class="flex min-h-10 items-center gap-2 sm:min-h-0">
          <input v-model="form.enabled" type="checkbox" id="img-enabled" />
          <label for="img-enabled" class="text-xs text-text-secondary">启用</label>
        </div>
      </div>
