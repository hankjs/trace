...
pub async fn handle_user_message(
    ...
    let session_id = ensure_session(state, topic_id, text).await?;
    ...
    let opts = ChatTurnOpts {
        ...
        auth_token: sign_handy_jwt(state).await,
...
async fn ensure_session(state: &Arc<AppState>, topic_id: &str, first_text: &str) -> Result<String> {
    if let Some(chat) = state.db.get_handy_chat(topic_id).await? {
        if state.db.get_session(&chat.session_id).await.ok().flatten().is_some() {
            return Ok(chat.session_id);
        }
        tracing::info!(topic_id, "handy 映射会话已删除，重建会话");
    }
    let session = create_handy_session(state).await?;
...
/// 建 handy 渠道会话：metadata 写 {"source":"handy"}（照 weixin/router.rs 的模式）。
pub async fn create_handy_session(state: &Arc<AppState>) -> Result<hank_db::Session> {
...
/// 为 handy 配置的 trace 用户签内部 JWT（spec 类工具回调 server 用）。
/// 未配置 handy.user_id 时返回空串：会话照常跑，spec 类工具回调会 401。
async fn sign_handy_jwt(state: &Arc<AppState>) -> String {
    let Some(user_id) = state.config.handy.as_ref().and_then(|c| c.user_id.clone()) else {
        return String::new();
    };
    let username = state
        .db
