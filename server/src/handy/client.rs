...
//!
//! client 是按账号（用户级 handy_accounts 行）现建现用的轻量封装，
//! 不挂 AppState：凭证热更新天然生效（每次从库里重读）。
...
/// 响应里的 resume_ref 不消费（trace 侧映射以 agent_interactions 表为准），
/// serde 忽略未知字段，不在结构体里承接。
...
impl HandyApi {
    /// base_url 调用方已去尾斜杠（这里再防御一次，直接构造也不出双斜杠）。
    ...
    /// 连通自检：token 是否有效 + handy 侧是否已配 webhook。
    /// 返回 data 原样（{token_id, token_name, identity_id, webhook_configured}）。
...
mod tests {
    ...
    fn new_trims_trailing_slash() {
        let api = HandyApi::new("https://handy.example.com/", "hnk_x");
        assert_eq!(api.base_url, "https://handy.example.com");
    }
}
