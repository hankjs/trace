//! handy webhook：交互单应答回推 + 话题留言推送的入口。
//!
//! handy 约定：POST 原始 body + `X-Handy-Signature: hex(HMAC-SHA256(webhook_secret, body))`
//! （纯 hex，无 `sha256=` 前缀），重试 3 次（1s/2s）后放弃。
//! - `interaction.answered`：闸门应答 → answer_and_resume；pusher 另有 30s 轮询兜底，
//!   两条路径共用 `answer_trace_interaction`（原子应答天然幂等，重复投递无害）。
//! - `message.created`：用户在外部话题的留言。**handy 收到 2xx 即标已读、不再重推**，
//!   所以 handler 只做验签/解析/幂等去重就快速返回，实际派发挪到异步 task
//!   （同步处理失败会把消息丢掉）。去重按 message_id（至少一次投递的重试都去重掉）。

use crate::handy::client::HandyMessage;
...
/// message.created 去重窗口：handy 重试都在几秒内到达，10 分钟绰绰有余
...
pub async fn webhook_handler(
    ...
    let Some(secret) = state
        ...
        return (StatusCode::SERVICE_UNAVAILABLE, "handy 未配置").into_response();
    };
    ...
    if !verify_signature(&secret, &body, signature) {
        tracing::warn!("handy webhook 验签失败");
    ...
    match payload.event_type.as_str() {
        "interaction.answered" => handle_interaction_answered(&state, &payload).await,
        "message.created" => handle_message_created(&state, payload),
        // 其他事件类型本阶段不消费，正常接收避免 handy 重试
        _ => StatusCode::OK.into_response(),
    }
}

async fn handle_interaction_answered(state: &Arc<AppState>, payload: &WebhookPayload) -> Response {
    let Some(trace_interaction_id) = extract_trace_interaction_id(payload) else {
        tracing::warn!(
            handy_interaction_id = %payload.interaction_id,
            "handy webhook 缺少 resume_ref.trace_interaction_id"
        );
        return (StatusCode::BAD_REQUEST, "missing trace_interaction_id").into_response();
    };

    answer_trace_interaction(state, &trace_interaction_id, &payload.answer).await;
    StatusCode::OK.into_response()
}

/// message.created：验签/解析/去重后立即 2xx，派发挪到异步 task。
/// handy 收到 2xx 就标已读，同步处理失败会把这条留言丢掉。
fn handle_message_created(state: &Arc<AppState>, payload: WebhookPayload) -> Response {
    ...
    {
        let mut dedupe = message_dedupe().lock().unwrap();
        if !dedupe.check_and_insert(&payload.message_id, Instant::now()) {
            return StatusCode::OK.into_response();
        }
    }
    let Some(api) = state.handy.clone() else {
        tracing::warn!("handy 已配置但未启用（无 client），message.created 无法派发");
        return (StatusCode::SERVICE_UNAVAILABLE, "handy 未启用").into_response();
    };
    ...
    tokio::spawn(async move {
        if let Err(e) = router::handle_user_message(&state, &api, &topic_id, &msg).await {
            tracing::warn!(
                topic_id,
                message_id = %msg.id,
...
/// 进程内带 TTL 的 message_id 去重集合。重试都在几秒内到达，不需要落表；
/// 重启丢窗口的风险 = 重启瞬间的重试可能重复派发一次，可接受。
...
impl MessageDedupe {
    ...
    fn check_and_insert(&mut self, id: &str, now: Instant) -> bool {
        self.sweep(now);
...
fn message_dedupe() -> &'static Mutex<MessageDedupe> {
    static DEDUPE: OnceLock<Mutex<MessageDedupe>> = OnceLock::new();
    DEDUPE.get_or_init(|| Mutex::new(MessageDedupe::new(DEDUPE_TTL)))
}

/// webhook 与兜底轮询共用的应答入口：调现成的 answer_and_resume。
/// channel_ctx 传 None（与 admin 手动应答同路径，绕开飞书卡片耦合）；
...
pub async fn answer_trace_interaction(
    ...
    match interaction_flow::answer_and_resume(state, trace_interaction_id, answer, "handy", None)
        .await
    {
        Ok(()) => tracing::info!(
...
mod tests {
    ...
    fn dedupe_rejects_replay_within_ttl() {
        let mut d = MessageDedupe::new(Duration::from_secs(600));
        let t0 = Instant::now();
        assert!(d.check_and_insert("msg_1", t0));
        // 窗口内的重试（handy 重试间隔 1s/2s）被去重
        assert!(!d.check_and_insert("msg_1", t0 + Duration::from_secs(3)));
        // 不同消息不受影响
        assert!(d.check_and_insert("msg_2", t0 + Duration::from_secs(3)));
    }
    ...
    fn dedupe_allows_after_ttl_and_sweeps() {
        ...
        assert!(d.check_and_insert("msg_1", t0));
        // 超出窗口后同一 id 视为新消息
        assert!(d.check_and_insert("msg_1", t0 + Duration::from_secs(601)));
        // sweep 清掉了过期项
